...
public class SKatexMathView extends View {
    ...
    private SKatexMathDrawable drawable;

    private float scale;
    private float left;
    private float top;

    public SKatexMathView(Context context) {
        super(context);
        init(context, null);
    }

    public SKatexMathView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.SKatexMathView);
        String fontName = array.getString(R.styleable.SKatexMathView_skmv_font);
        try {
            final Drawable background;
            {
                final int res = array.getResourceId(R.styleable.SKatexMathView_skmv_background, 0);
                if (res != 0) {
                    final String type = context.getResources().getResourceTypeName(res);
                    if ("drawable".equals(type)) {
                        background = array.getDrawable(R.styleable.SKatexMathView_skmv_background);
                    } else if ("color".equals(type)) {
                        background = new ColorDrawable(array.getColor(R.styleable.SKatexMathView_skmv_background, 0));
                    } else {
            ...
            this
                    .textSize(array.getDimensionPixelSize(R.styleable.SKatexMathView_skmv_textSize, 0))
                    .textColor(array.getColor(R.styleable.SKatexMathView_skmv_textColor, Color.black.getColorInt()))
                    .background(background)
                    .align(
                            array.getInteger(R.styleable.SKatexMathView_skmv_alignVertical, ALIGN_START),
                            array.getInteger(R.styleable.SKatexMathView_skmv_alignHorizontal, ALIGN_START)
                    );
        } finally {
            array.recycle();
        }

            SKatexMathAndroid.init(context);
        ...
        Log.d("character", "drawChars: " + latex);
    }

    @NonNull
    public SKatexMathView textSize(@Px int textSize) {
        this.textSize = textSize;
        return this;
    }

    @NonNull
    public SKatexMathView textColor(@Px int textColor) {
        this.textColor = textColor;
        return this;
    }

    @NonNull
    public SKatexMathView background(@Nullable Drawable background) {
        this.background = background;
        return this;
    }

    @NonNull
    public SKatexMathView align(@Align int alignVertical, @Align int alignHorizontal) {
    ...
    public SKatexMathView setLatex(@NonNull String latex) {

        this.latexText=latex;
        final SKatexMathDrawable drawable = SKatexMathDrawable.builder(latexText)
                .textSize(textSize)
                .color(textColor)
                .background(background)
                .build();
    ...
    public void setLatexDrawable(@NonNull SKatexMathDrawable drawable) {
