...
import shakiba.enur.skatexmath.SKatexMathAndroid;
...
public class DefaultTeXFontParser {
    ...
    public DefaultTeXFontParser() throws ResourceParseException {
        this(SKatexMathAndroid.getResourceAsStream(RESOURCE_NAME), RESOURCE_NAME);
    }
    ...
    public FontInfo[] parseFontDescriptions(FontInfo[] fi) throws ResourceParseException {
        ...
        if (fontDescriptions != null) { // element present
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (base == null) {
                    fi = parseFontDescriptions(fi, SKatexMathAndroid.getResourceAsStream(include), include);
                } else {
                    fi = parseFontDescriptions(fi, SKatexMathAndroid.getResourceAsStream(include), include);
                }
            }
    ...
    protected void parseExtraPath() throws ResourceParseException {
        ...
        if (syms != null) { // element present
            ...
            SymbolAtom.addSymbolAtom(SKatexMathAndroid.getResourceAsStream(include), include);
        }
        ...
        if (settings != null) { // element present
            ...
            TeXFormula.addSymbolMappings(SKatexMathAndroid.getResourceAsStream(include), include);
        }
    }
    ...
    public static Font createFont(@NonNull String path) {
        // todo: cache
        final Typeface typeface = SKatexMathAndroid.loadTypeface(path);
    ...
    public Map<String,CharFont> parseSymbolMappings() throws ResourceParseException {
        ...
        if (symbolMappings == null)
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                try {
                    if (base == null) {
                        map = factory.newDocumentBuilder().parse(SKatexMathAndroid.getResourceAsStream(include)).getDocumentElement();
                    } else {
                        map = factory.newDocumentBuilder().parse(SKatexMathAndroid.getResourceAsStream(include)).getDocumentElement();
                    }
                } catch (Exception e) {
